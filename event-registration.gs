// Paste this ENTIRE file into the EXISTING event registration Apps Script project.
...
const FestivalRegistration = (() => {
  ...
  const events = {
    ...
    thali: {title:"Pooja Thali Decoration", date:"20 September 2026", sheet:"Pooja Thali", rules:["Theme: Eco-Friendly Green. Use natural, biodegradable materials available at home; reuse creatively. Avoid plastic, thermocol and non-biodegradable materials.", "Decorate at home and bring the completed thali to the Clubhouse by 6:00 PM, ready for display.", "Three prizes: first, second and third.",judging]},
...
// SETTINGS: update the existing event registration project. For a fresh setup,
// create a separate project; do not replace the Aarti/accounts scripts.
...
// Deadlines are fixed in FestivalRegistration; obsolete FANCY_DRESS_DEADLINE properties are ignored.
...
function eventSettings() {
  ...
  return {drawingMaxAge:Number(p.getProperty("DRAWING_MAX_AGE")||18), musicUrl:p.getProperty("TALENT_MUSIC_URL")||"", fancyDeadline:FestivalRegistration.deadline("fancy"), registrationRevision:"2026-09-17"};
}
function eventBook() {
...
function eventSheet(book,name) {
  ...
  const columns=name==="Blood Donation"?[...EVENT_COLUMNS,"Have donated blood before?"]:EVENT_COLUMNS;
  ...
  if(headers.join("|")!==columns.join("|"))throw Error("Registration tab headers have changed. Please contact the committee.");
  return sheet;
}
...
function doPost(e) {
  ...
  try {
    ...
    if(FestivalRegistration.registrationStatus(d.event).closed)throw Error(definition.title+" nominations are closed.");
    ...
    const values=[new Date(),d.requestId,key,FestivalRegistration.text(d.firstName),FestivalRegistration.text(d.lastName),d.wing,d.flatNo,FestivalRegistration.phone(d.phone),d.age||"",definition.title,JSON.stringify(d),"Yes",definition.rules.includes(FestivalRegistration.gift)?FestivalRegistration.gift:"",d.performanceType||"",d.groupName||"",d.adults||"",d.children||"",d.event==="funfair"?1:"",d.event==="funfair"?500:"",d.event==="funfair"?"Not verified":"",category,d.actType||"",d.performanceTitle||"",d.durationSeconds||"",d.teamName||"",d.stallName||"",d.stallDetails||"",d.costume||"",d.dishName||"",d.ingredients||"",...Array.from({length:6},(_,i)=>{const p=(d.players||[])[i]||{};return [p.firstName||"",p.lastName||"",p.phone?FestivalRegistration.phone(p.phone):""];}).flat()];
    if(d.event==="blood")values.push(d.donatedBefore?"Yes":"No");
...
function refreshChildrenGifts() {
  ...
  try {
    ...
    for(const id of ["drawing","talent","fancy"]) {
